...
URLの書き方（例外なく厳守）:
- URLを出すときは必ず [ラベル](<URL>) の形式にする。URLは必ず < > で囲む（Discordのカード展開を防ぐ）
- ラベルは内容が分かる短い日本語にする（例: [軍配の販売ページ](<https://brain-market.com/...>)、[あおいさんのポスト](<https://x.com/...>)）
- ラベルにURL文字列を入れるのは禁止（悪い例: [https://brain-market.com/...](<...>)）
- 素のURLを地の文にそのまま書くのも禁止。必ず [ラベル](<URL>) のリンク形式にする
""".strip()
