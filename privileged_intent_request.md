...
> ⚠️ 実装が前提。下の「未解決」を読むこと。未実装のまま Yes と答えない。
...
`main.py` の `fetch_messages()` は対象チャンネルの全メッセージを無条件に集める。
特定ユーザーを除外する仕組みはコード上に存在しない。一方、公開ポリシー §7 では
「データ処理からの除外」を受け付けると書いている。

このまま「オプトアウトできる＝Yes」と答えると、プライバシーポリシーの§6が
実態と食い違っていた 2026-07-28 の件と同じ構図になる。

対応案：`servers.json` に `exclude_user_ids` を持たせ、`fetch_messages()` で
そのIDのメッセージを落とす。数行で済み、既存の動作には影響しない
（リストが空なら現状と完全に同じ挙動）。実装すれば Yes が事実になる。

### Privacy Policy URL がポータル未登録

「Botプロフィールからリンクされている」と答える前に、General Information へ登録する。
