...
- [x] **Privacy Policy URL をポータルへ登録** — 確認したところ登録済みだった（2026-07-28）
- [x] **オプトアウトの実装** — `exclude_user_ids` を実装済み（2026-07-28）
- [x] **スクリーンショット撮影** — `intent_evidence/` に5点（最後のセクション参照）
- [ ] **各サーバーのルール/お知らせチャンネルにポリシーのリンクをピン留め**
      → 「Botプロフィールからリンク、かつ各サーバーにピン留め」と答えるため、
        ピン留めしないなら回答文からその一節を削ること
...
**撮影済み**：`intent_evidence/`（gitignore済み。メンバー名が写るためコミットしない）。
提出時はGoogle Driveの共有リンクか、どこかに置いた直リンクを貼る。

| ファイル | 用途 | 写っているもの |
|---|---|---|
| `01_message-content_daily-digest_meikyo.jpg` | Message Content | 明鏡 #日々のまとめ の日次ダイジェスト。投稿者・時刻・対象チャンネル名つき |
| `02_message-content_daily-digest_entsumugi.jpg` | Message Content | 縁紡 #デイリートピック の同機能（複数サーバーで動いている証拠） |
| `03_server-members_metrics-report.jpg` | Server Members | ラウンジの指標レポート。参加者9,923／購入者3,125／ロール2,593と前日比・7日推移表 |
| `04_server-members_trend-chart_and_sales-message.jpg` | 両方 | 推移グラフ＋直上に「累計 3123 部」の販売通知。**Feature 2 が本文から数値を1つ抜いている根拠がそのまま写っている** |
| `05_manual-workaround_reminder-dm.jpg` | Server Members 補強 | 手入力を回すためのリマインドDMと返信。プロフィールカードに「InfoSearchBot／5個の共通サーバー」も写っており、非公開Botであることの裏づけになる |

`servers.json`（読み取り対象を明示列挙している設定ファイル）のスクショを足すとさらに強い。
必要ならエディタで開いて撮る。
